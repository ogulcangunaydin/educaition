...
def estimate_ranking_from_score(score: float, area: str, distribution_data: Dict) -> Optional[int]:
    """
...
def filter_programs_by_ranking_and_area(programs: List[Dict],
                                        expected_ranking: int,
                                        area: str,
                                        alternative_ranking: Optional[int] = None,
    ...
    def check_city_match(program_city: str, city_prefs: List[str]) -> bool:
        ...
        for pref_city in city_prefs:
            ...
            if pref_city_lower == 'istanbul' and 'İSTANBUL' in city:
                return True
            elif pref_city_lower == 'ankara' and 'ANKARA' in city:
                return True
            elif pref_city_lower == 'izmir' and 'İZMİR' in city:
                return True
            elif pref_city_lower == 'other':
                if 'İSTANBUL' not in city and 'ANKARA' not in city and 'İZMİR' not in city:
                    return True
        return False
    ...
    def filter_with_options(check_city: bool, check_language: bool, 
                           include_alt_area: bool, max_expansion: float) -> List[Dict]:
        """
        ...
        for program in programs:
            ...
            if is_main_area:
                ...
                check_ranking = alternative_ranking if alternative_ranking else expected_ranking
            ...
            if check_language and preferred_language:
                if not check_language_match(program.get('program_detail', ''), preferred_language):
                    continue
    ...
    logging.warning(f"Using ultimate fallback - returning closest programs by ranking")
    ...
    for program in programs:
        ...
        check_ranking = expected_ranking if is_main_area else (alternative_ranking or expected_ranking)
    ...
    all_area_programs.sort(key=lambda x: (
        -x['university_priority'],
        x['ranking_distance']
    ))
...
def get_suggested_programs(riasec_scores: Dict[str, float],
                           ...
                           distribution_json_path: str = None) -> Dict:
    """
    ...
    suggested_jobs = find_top_matching_jobs(riasec_scores, top_n=3, csv_path=riasec_csv_path)
    ...
    if alternative_score and alternative_area:
        alternative_ranking = estimate_ranking_from_score(alternative_score, alternative_area, distribution_data)
    ...
    for suggestion in suggested_programs:
        ...
        for fp in filtered_programs:
            if (suggestion.get('program', '').lower() in fp.get('program', '').lower() and
                suggestion.get('university', '').lower() in fp.get('university', '').lower()):
                matching_program = fp
                break
