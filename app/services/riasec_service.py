...
def load_job_riasec_scores_from_db(db: Session) -> dict[str, dict[str, float]]:
    """
    Load job RIASEC scores from database.
    Returns: {job_title: {R: score, I: score, ...}}
    """
    # Import here to avoid circular import
    from app.modules.reference_data.models import RiasecJobScore

    jobs = {}
    try:
        records = db.query(RiasecJobScore).all()
        for record in records:
            jobs[record.job_title] = record.to_dict()
    except Exception as e:
        logging.error(f"Error loading job RIASEC scores from DB: {e}")
        jobs = {}
...
def calculate_cosine_similarity(
    student_scores: dict[str, float], job_scores: dict[str, float]
) -> float:
    """
...
def calculate_holland_code_match(
    student_scores: dict[str, float], job_scores: dict[str, float]
) -> float:
    """
...
def send_request_to_gpt(prompt: str) -> str:
    ...
    if response.status_code == 200:
        ...
        logging.error(
            f"Failed to get response from GPT. Status code: {response.status_code}"
        )
        return None
...
def get_program_suggestions_from_gpt(
    jobs: list[dict],
    available_programs: list[dict],
    desired_universities: list[str] = None,
                ...
                for i, job in enumerate(jobs):
                    ...
                    for gpt_program in gpt_programs:
                        ...
                        for p in available_programs:
                            ...
                            if (
                                program_name in actual_name
                                or actual_name in program_name
                            ):
