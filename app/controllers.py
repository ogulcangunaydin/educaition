...
from . import db_operations, models, schemas, security
...
def create_user(user: schemas.UserCreate, db: Session = None):
    ...
    existing_user = (
        ...
        .filter(
            (models.User.username == clean_name) | (models.User.email == clean_email)
        )
        .first()
    )
...
def update_user(user_id: int, user: schemas.UserCreate, db: Session = None):
    ...
    existing_user = (
        db.query(models.User)
        .filter(models.User.username == cleaned_username, models.User.id != user_id)
        .first()
    )
...
def create_player(player_name, room_id, db: Session):
    ...
    existing_player = (
        db.query(models.Player)
        .filter(
            models.Player.player_name == clean_name, models.Player.room_id == room_id
        )
        .first()
    )
    if existing_player:
...
def start_game(room_id: int, name: str, db: Session, background_tasks: BackgroundTasks):
    ...
    if len(players) < 2:
        raise HTTPException(
            status_code=400, detail="At least two players are required to start a game"
        )
...
def create_dissonance_test_participant(
    db: Session, participant: schemas.DissonanceTestParticipantCreate
):
...
def update_dissonance_test_participant_personality_traits(
    participant_id: int, answers: str, db: Session
):
    # Retrieve the player from the database
    db_participant = (
        db.query(models.DissonanceTestParticipant)
...
def create_high_school_room(
    high_school_name: str, high_school_code: str, request: Request, db: Session
):
    user_id = request.session["current_user"]["id"]
...
def update_student_step1(
    student_id: int, data: schemas.ProgramSuggestionStudentUpdateStep1, db: Session
):
    student = (
        db.query(models.ProgramSuggestionStudent)
        .filter(models.ProgramSuggestionStudent.id == student_id)
...
def update_student_step2(
    student_id: int, data: schemas.ProgramSuggestionStudentUpdateStep2, db: Session
):
    student = (
        db.query(models.ProgramSuggestionStudent)
        .filter(models.ProgramSuggestionStudent.id == student_id)
...
def update_student_step3(
    student_id: int, data: schemas.ProgramSuggestionStudentUpdateStep3, db: Session
):
    student = (
        db.query(models.ProgramSuggestionStudent)
        .filter(models.ProgramSuggestionStudent.id == student_id)
...
def update_student_step4(
    student_id: int, data: schemas.ProgramSuggestionStudentUpdateStep4, db: Session
):
    student = (
        db.query(models.ProgramSuggestionStudent)
        .filter(models.ProgramSuggestionStudent.id == student_id)
...
def update_student_riasec(
    student_id: int, data: schemas.ProgramSuggestionStudentUpdateRiasec, db: Session
):
    student = (
        db.query(models.ProgramSuggestionStudent)
        .filter(models.ProgramSuggestionStudent.id == student_id)
